...
class TideApp:
    ...
    def create_ui(self):
        ...
        self.canvas_win = self.canvas_main.create_window((0, 0), window=self.scrollable_frame, anchor="nw")
        self.canvas_main.configure(yscrollcommand=scrollbar.set)
        self.canvas_main.bind("<Configure>", lambda e: self.canvas_main.itemconfig(self.canvas_win, width=e.width))
    ...
    def render_all(self):
        ...
        # Top row : 3 cadres en grid pour remplir toute la largeur
        top = tk.Frame(self.scrollable_frame, bg="#0f172a")
        top.pack(fill="x", pady=10, padx=8)
        top.columnconfigure(0, weight=1)
        top.columnconfigure(1, weight=1)
        top.columnconfigure(2, weight=1)

        h = 420  # hauteur fixe pour les 3 cadres

        # Calendrier
        cal = tk.Frame(top, bg="#1e293b", relief="solid", bd=1, height=h)
        cal.grid(row=0, column=0, padx=8, sticky="nsew")
        cal.grid_propagate(False)
        self.draw_calendar(cal)

        # Horloge
        clock = tk.Frame(top, bg="#1e293b", relief="solid", bd=1, height=h)
        clock.grid(row=0, column=1, padx=8, sticky="nsew")
        clock.grid_propagate(False)
        self.draw_clock(clock)

        # Marées du jour
        events = tk.Frame(top, bg="#1e293b", relief="solid", bd=1, height=h)
        events.grid(row=0, column=2, padx=8, sticky="nsew")
