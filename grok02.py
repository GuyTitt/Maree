...
class TideApp:
    ...
    def create_ui(self):
        ...
        header.pack(fill="x")
        tk.Label(header, text="🌊 Marées", font=("Segoe UI", 18, "bold"), fg="#38bdf8", bg="#1e293b").pack(side="left",
                                                                                                          padx=20,
                                                                                                          pady=12)
        ...
        self.date_label = tk.Label(nav, text=self.current_date.strftime("%Y-%m-%d"), fg="#e2e8f0", bg="#1e293b",
                                   width=12)
    ...
    def render_all(self):
        ...
        cal_frame = tk.Frame(top, bg="#1e293b", relief="solid", bd=1)
        cal_frame.pack(side="left", padx=8, fill="y")
        self.draw_clickable_calendar(cal_frame)

        # Horloge
        clock_f = tk.Frame(top, bg="#1e293b", relief="solid", bd=1)
        clock_f.pack(side="left", padx=8, fill="y")
        tk.Label(clock_f, text="Horloge de marée", bg="#1e293b", fg="#94a3b8", font=("Segoe UI", 9, "bold")).pack(
        ...
        tk.Label(ev_f, text="Marées du jour", bg="#1e293b", fg="#94a3b8", font=("Segoe UI", 9, "bold")).pack(anchor="w",
                                                                                                             padx=15,
                                                                                                             pady=8)
        ...
        self.canvas_widget.mpl_connect("motion_notify_event", self.on_mouse_move)
        plt.close(self.fig)

    def draw_clickable_calendar(self, parent):
        year = self.current_date.year
        month = self.current_date.month
        today = datetime.now().date()

        tk.Label(parent, text=self.current_date.strftime("%B %Y"), bg="#1e293b", fg="#e2e8f0",
        ...
        header.pack()
        for d in days:
            tk.Label(header, text=d, width=3, bg="#1e293b", fg="#64748b").pack(side="left")
        ...
        row.pack()
        col = 0
        for _ in range(first_day):  # jours vides
            tk.Label(row, text="", width=3, height=2, bg="#1e293b").pack(side="left")
            col += 1
        ...
        for day in range(1, days_in_month + 1):
            ...
            bg = "#0284c7" if d == self.current_date else "#1e293b"
            fg = "white" if d == self.current_date else "#e2e8f0"
            lbl = tk.Label(row, text=str(day), width=3, height=2, bg=bg, fg=fg, relief="flat", cursor="hand2")
            lbl.pack(side="left")
    ...
    def on_mouse_move(self, event):
        if not event.inaxes or not self.data: return
        ...
        times = [datetime.fromisoformat(d["time"].replace("Z", "")) for d in self.data]
        heights = [d["height"] for d in self.data]
        idx = min(range(len(times)), key=lambda i: abs(times[i].timestamp() - x))

        time_str = times[idx].strftime("%H:%M")
        height_str = f"{heights[idx]:.2f} m"

        # Affichage tooltip simple dans la console pour l'instant (on peut améliorer avec annotation)
        print(f"→ {time_str}   {height_str}", end="\r")
