...
class TideApp:
    ...
    def redraw_clock(self) -> None:
        ...
        # ── Arcs PM / BM : 4 marées futures max, chevauchement géré ─
        # On n'affiche que les événements à venir (ou en cours) sur 24h.
        now_ts  = now.timestamp()
        # Filtrage : conserver les 4 prochains événements (actuels ou futurs)
        future_evs = [
            ev for ev in (getattr(self, "events_all", self.events))
            if parse_local(ev["time"]).timestamp() >= now_ts - 3600
        ][:4]

        def _hour_angle(dt: datetime) -> float:
            """Angle en degrés sur l'horloge 24h (0° = minuit = haut)."""
            mins = dt.hour * 60 + dt.minute + dt.second / 60
            return (mins / 1440) * 360 - 90   # -90 : minuit au sommet

        # Détection de chevauchement entre arcs consécutifs (< 30 min d'écart)
        OVERLAP_THRESH = 30 * 60   # 30 minutes en secondes
        ...
        for i in range(len(future_evs) - 1):
            ...
            ang_a = _hour_angle(dt_a)
            ang_b = _hour_angle(dt_b)
            col   = BM_COL if ev_a["type"] == "PM" else PM_COL

            # Vérifie si cet arc chevauche le précédent ou le suivant
            overlap_prev = (i > 0 and
                abs(parse_local(future_evs[i]["time"]).timestamp()
                    - parse_local(future_evs[i-1]["time"]).timestamp()) < OVERLAP_THRESH)
            overlap_next = (i < len(future_evs) - 2 and
                abs(parse_local(future_evs[i+1]["time"]).timestamp()
                    - parse_local(future_evs[i]["time"]).timestamp()) < OVERLAP_THRESH)
        ...
        ar = r - 8
        for arc in arcs:
            w  = 3 if arc["thin"] else 6
            ex = -(arc["ang_b"] - arc["ang_a"])   # extent négatif = sens horaire Tkinter
            c.create_arc(cx - ar, cy - ar, cx + ar, cy + ar,
                         start=-arc["ang_a"], extent=ex,
                         style="arc", outline=arc["col"], width=w)

        # ── Bulles d'événement avec trait de rappel ────────────────────
        # Chaque événement futur reçoit une bulle à l'horizontale du cadran.
        BUBBLE_X_RIGHT = cx + r + 14   # bulles à droite pour i pairs
        BUBBLE_X_LEFT  = cx - r - 14   # bulles à gauche pour i impairs

        for idx, ev in enumerate(future_evs):
            dt      = parse_local(ev["time"])
            ang_deg = _hour_angle(dt)
            ang_rad = math.radians(ang_deg)
            col     = PM_COL if ev["type"] == "PM" else BM_COL

            # Point d'ancrage sur le cadran
            tip_x = cx + math.cos(ang_rad) * (r + 4)
            tip_y = cy + math.sin(ang_rad) * (r + 4)

            # Côté de la bulle : pair = droite, impair = gauche
            right_side = (math.cos(ang_rad) >= 0)
            bx = cx + r + 12 if right_side else cx - r - 12
            by = tip_y

            # Trait de rappel
            c.create_line(tip_x, tip_y, bx, by, fill=col, width=1, dash=(3, 2))

            # Bulle (rectangle arrondi via polygon smooth)
            pad_x, pad_y = 4, 2
            label_txt = f"{ev['type']} {dt.strftime('%H:%M')}"
            # Estimer largeur texte (~5.5 px/car en font 7)
            bw = len(label_txt) * 5.5 + pad_x * 2
            bh = 14
            if right_side:
                bx0, bx1 = bx, bx + bw
            else:
        ...
        # (Les étiquettes PM/BM sont désormais dans les bulles latérales)
    ...
    def draw_events(self, parent: tk.Frame) -> None:
        """Affiche les événements PM/BM sur les deux journées J et J+1.

        Les événements sont regroupés par jour avec un séparateur de date.
        En mode « aujourd'hui », le titre indique « Prochaines marées ».

        Args:
            parent: Frame carte des marées.
        """
        today = datetime.now(PARIS).date()
        title = "Prochaines marées" if self.current_date == today else "Marées J + J+1"
        ...
        for ev in all_evs:
            ...
            # Séparateur de date à chaque nouveau jour
            if ev_date not in days_seen:
                days_seen.add(ev_date)
                label_day = (
                    "Aujourd'hui" if ev_date == today
                    else "Demain"  if ev_date == today + timedelta(days=1)
                    else f"{day_names[ev_date.weekday()]} {ev_date.strftime('%d/%m')}"
                )
                sep = tk.Frame(parent, bg=BORDER, height=1)
                sep.pack(fill="x", padx=10, pady=(6, 2))
                tk.Label(parent, text=label_day, fg=ACCENT, bg=SURFACE,
                         font=(FONT, 8, "bold")).pack(anchor="w", padx=12, pady=(0, 2))
            ...
            tk.Label(inner, text=ev["type"], bg=SURFACE, fg=color,
    ...
    def draw_chart(self, parent: tk.Widget) -> None:
        ...
        * Ligne verticale de survol + bulle d'info (heure, hauteur).
        ...
        today = datetime.now(PARIS).date()
        chart_title = (
            "MARÉES — Aujourd'hui + Demain"
            if self.current_date == today
            else (f"MARÉES — {self.current_date.strftime('%d/%m/%Y')}"
        ...
        legend_patches = [
            mpatches.Patch(color=PM_COL,   label="Montant"),
            mpatches.Patch(color=BM_COL,   label="Descendant"),
            mpatches.Patch(color="#f472b6", label="Maintenant"),
        ]
