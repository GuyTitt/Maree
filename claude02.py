...
class TideApp:
    ...
    def draw_chart(self, parent):
        ...
        # Build local datetime list (keep the local offset as-is)
        times   = [parse_local(d["time"]) for d in self.data]
        ...
        if self.current_date == today:
            ...
            for i in range(len(self.data)-1):
                t0 = parse_local(self.data[i]["time"])
                t1 = parse_local(self.data[i+1]["time"])
                if t0 <= now_local <= t1:
