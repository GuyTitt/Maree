...
class TideApp:
    ...
    def redraw_clock(self):
        ...
        # ── Zone arcs (correct colors!) ───────────────────────────────────────
        # The tide clock: top = PM, bottom = BM
        # Right half (0° to 180° in tkinter = east to west via south): descending → orange BM
        # Left half (180° to 360° = west to east via north): ascending → cyan PM
        ar = r - 16
        # Right arc: descent (PM→BM), orange
        c.create_arc(cx-ar, cy-ar, cx+ar, cy+ar,
                     start=-90, extent=-180, style="arc",
                     outline=BM_COL, width=9)
        # Left arc: ascent (BM→PM), cyan
        c.create_arc(cx-ar, cy-ar, cx+ar, cy+ar,
                     start=90, extent=-180, style="arc",
                     outline=PM_COL, width=9)

        # ── Hour graduations with time labels ─────────────────────────────────
        # Tide clock cycle ≈ 12h25min. We show 12 sectors (each ≈ 1h).
        # Noon (PM) at top (270° in standard, -90° tkinter start).
        # We compute tide-hours relative to PM start.
        # For display: find the time of the first PM event to anchor labels.
        pm_events = [e for e in self.events if e["type"] == "PM"]
        first_pm_dt = parse_local(pm_events[0]["time"]) if pm_events else None

        for i in range(24):
            ang_deg = i * 15 - 90          # 0 = top, clockwise
            ang     = math.radians(ang_deg)
            is_major = (i % 2 == 0)
            tick_in  = r - (15 if is_major else 8)
            ...
            # Hour label every 2 ticks (= every 60 min of tide cycle)
            if is_major and first_pm_dt:
                # i=0 → PM, i=2 → +1h, … i=12 → BM, …
                tide_hours = i // 2
                label_dt   = first_pm_dt + timedelta(hours=tide_hours)
                label_str  = label_dt.strftime("%H:%M") if tide_hours < 13 else ""
                if label_str:
                    lx = cx + math.cos(ang) * (r - 28)
                    ly = cy + math.sin(ang) * (r - 28)
                    c.create_text(lx, ly, text=label_str, fill="#475569",
                                  font=(FONT, 6))

        # ── PM / BM fixed labels ──────────────────────────────────────────────
        c.create_text(cx, cy - r + 22, text="PM", fill=PM_COL,
                      font=(FONT, 9, "bold"))
        c.create_text(cx, cy + r - 22, text="BM", fill=BM_COL,
                      font=(FONT, 9, "bold"))
        ...
        # Shadow (use dark solid color, no alpha)
        ...
        if state["is_today"] and state["current_height"] is not None:
            ...
            c.create_text(cx, cy + r + 12, fill=ACCENT,
                          text=f"{h_val:.2f} m", font=(FONT, 13, "bold"))
            c.create_text(cx, cy + r + 30, fill=dir_col,
                          text=dir_label, font=(FONT, 8))
            c.create_text(cx, cy + r + 46, fill=MUTED,
                          text=now_s, font=(FONT, 8))
        else:
            c.create_text(cx, cy + r + 18, fill=MUTED,
                          text="Horloge – première marée du jour",
                          font=(FONT, 7))
